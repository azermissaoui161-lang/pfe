...
const cancelOrder = async (req, res) => {
  try {
    ...
    await AuditLog.create({
      ...
      entity: 'ORDER',
      entityId: order._id,
      details: { orderNumber: order.orderNumber, newStatus: 'annule' },
      ipAddress: req.ip
    });

    res.json({ success: true, data: order, message: 'Commande annulée avec succès' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};

// @desc    Générer un devis pour une commande
...
const generateQuote = async (req, res) => {
  try {
    ...
    const quote = {
      quoteNumber: `DEV-${order.orderNumber}`,
    ...
    res.json({ success: true, data: quote, message: 'Devis généré avec succès' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
