...
exports.getVariance = async (req, res) => {
  try {
    ...
    const variancePercentage = budget.budget > 0
      ? ((variance / budget.budget) * 100).toFixed(2)
      : 0;
    ...
    res.json({
      ...
      data: {
        ...
        status: variance >= 0 ? 'sous_budget' : 'sur_budget'
      }
    });
  } catch (error) {
    handleError(error, res, 'Erreur lors de la récupération de l\'écart budgétaire');
  }
};
...
exports.getSummary = async (req, res) => {
  try {
    ...
    budgets.forEach(b => {
      ...
      summary.totalBudgeted += b.budget;
      summary.totalActual += b.actual;
      summary.totalVariance += variance;
      if (variance >= 0) {
        summary.underBudget += 1;
      } else {
    ...
    handleError(error, res, 'Erreur lors de la récupération du résumé budgétaire');
  }
};
