...
const cancelPayment = async (req, res) => {
  try {
    ...
    if (payment.status === 'annule') {
      return res.status(400).json({ success: false, message: 'Paiement déjà annulé' });
    }

    if (payment.status === 'validé') {
      return res.status(400).json({ success: false, message: 'Impossible d\'annuler un paiement validé' });
    }
    ...
    await AuditLog.create({
      ...
      entity: 'PAYMENT',
      entityId: payment._id,
      details: { paymentNumber: payment.paymentNumber, newStatus: 'annule' },
      ipAddress: req.ip
    });

    res.json({ success: true, data: payment, message: 'Paiement annulé avec succès' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
// @route   GET /api/payments/export/:format
