...
function StockAdmin(){
  ...
  // Données
  const [cat,setCat]=useState(IC),[prod,setProd]=useState(IP),[mov,setMov]=useState(IM),[supp,setSupp]=useState(IS),[reports,setReports]=useState(IR)
  ...
  return <div className="stock-container">
            ...
            {fc.length
              ? fc.map(c=><article key={c.id} className="category-card">
                  ...
                  <div className="category-actions">
                    ...
                    <button className="btn-icon" onClick={()=>hdlDelCat(c.id,c.name)}>🗑️</button>
                  </div>
                </article>)
              ...
              ? fr.map(r=><article key={r.id} className="report-card">
                  ...
                  <div className="report-content">
                    ...
                    <div className="report-actions" style={{display:"flex",flexWrap:"wrap",gap:"8px"}}>
                      <button className="btn-outline" title="Voir le contenu" onClick={()=>hdlViewReport(r)}>👁 Voir</button>
                      <button className="btn-outline" title="Modifier" onClick={()=>hdlEditReport(r)}>✏️ Modifier</button>
                      <button className="btn-outline" title="Télécharger PDF" onClick={()=>hdlDownloadReport(r)}>⬇️ PDF</button>
                      <button className="btn-outline btn-danger-outline" title="Supprimer" onClick={()=>hdlDelReport(r.id)}>🗑️ Supprimer</button>
                    </div>
                  </div>
                </article>)
    ...
    <Modal
      ...
      onConfirm={er?hdlUpdReport:hdlAddReport}
